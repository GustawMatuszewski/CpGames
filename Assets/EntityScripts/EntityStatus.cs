...
public class EntityStatus : MonoBehaviour
{
    ...
    void UpdateDepressionState(float dt){
        ...
        if(mentalHealth <= suicidalThreshold){
            currentDepressionState = DepressionState.Suicidal;
            HandleSuicidalState(dt);
            return;
        }
        if(mentalHealth <= deepDepressionThreshold){
    ...
    bool CheckEnemyInFOV(){
        ...
        foreach(Collider col in hits){
            ...
            if(Vector3.Angle(fwd, dir) < halfFOV){
                ...
                if(!Physics.Raycast(pos, dir, dist, obstacleMask))
                    return true;
            }
        }
        return false;
    ...
    void SetMood(Mood mood, bool condition){
        if(condition){
            if(moodSet.Add(mood)) moods.Add(mood);
        }
        else{
            if(moodSet.Remove(mood)) moods.Remove(mood);
        }
    }
        ...
        foreach(FoodItem.Effect applyEffects in effects){
            switch(applyEffects){
                ...
                case FoodItem.Effect.diareah:
                    ...
                    diareahTime += 40f;
                    if(debugMode) Debug.Log("diareah");
    ...
    IEnumerator PoisonRoutine(){
        while(poisonTime > 0f){
            entityHealth = Mathf.Clamp(entityHealth - 0.05f, 0f, entityMaxHealth);
            poisonTime -= 1f;
    ...
    IEnumerator NauseaRoutine(){
        ...
        while(nauseaTime > 0f){
            ...
            float offset = Mathf.Sin(Time.time * 2f) * Mathf.SmoothStep(0f, 1f, t);
    ...
    IEnumerator IllRoutine(){
        while(illTime > 0f){
            entityHealth = Mathf.Clamp(entityHealth - 0.03f, 0f, entityMaxHealth);
            entityThirst = Mathf.Clamp(entityThirst - 2f,    0f, entityMaxThirst);
            illTime -= 1f;
            yield return new WaitForSeconds(1f);
        }
        illCoroutine = null;
    ...
    IEnumerator DiareahRoutine(){
        while(diareahTime > 0f){
            entityThirst = Mathf.Clamp(entityThirst - 3f, 0f, entityMaxThirst);
            entityHunger = Mathf.Clamp(entityHunger - 1f, 0f, entityMaxHunger);
            diareahTime -= 1f;
            yield return new WaitForSeconds(1f);
        }
    ...
    IEnumerator DrunkRoutine(){
        while(drunkTime > 0f){
            mentalHealth = Mathf.Clamp(mentalHealth - 0.2f, 0f, 100f);
            drunkTime -= 1f;
            yield return new WaitForSeconds(1f);
        }
    ...
    public void LimbTracker(){
        for(int i = 0; i < limbs.Count; i++){
            ...
            if(crippled){
                if(limb.health > 0f){
                    limb.health = 0f;
                    if(debugMode) Debug.Log((limb.severed ? "severed" : "fractured") + ": " + limb.name);
                }
