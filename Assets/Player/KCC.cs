...
public class KCC : MonoBehaviour
{
    ...
    [SerializeField] private EntityStatus entityStatus; // assign in inspector or auto-found
    ...
    public enum State
    {
        None, Idle, Air, Walk, Run, Sprint,
        Crouch, Prone, Climbing, Hanging, Attack, Dash
    }
    ...
    private void Start()
    {
        ...
        if (entityStatus == null)
        {
            ...
            if (entityStatus == null && debugMode)
                Debug.LogWarning("KCC: No EntityStatus found on " + gameObject.name + ". Stat effects will be skipped.");
        }
    }
    ...
    void FixedUpdate()
    {
        ...
        // Tell EntityStatus what state we are in so it can drain stamina correctly
        entityStatus?.ReportState(state);
    ...
    void StateController()
    {
        ...
        // EntityStatus blocks sprint when stamina is exhausted
        bool sprintAllowed = entityStatus == null || entityStatus.CanSprint;
        ...
        // Resolve raw base speed for this frame
        float rawSpeed;
        ...
        {
            state     = State.Crouch;
            rawSpeed  = crouchSpeed;
            capsuleHeight = crouchHeight;
        }
        else if (sprintHeld && sprintAllowed && moveInput.y > 0.1f)
        {
            state     = State.Sprint;
            rawSpeed  = sprintSpeed;
            capsuleHeight = standingHeight;
        }
        else if (run)
        {
        ...
        // Apply EntityStatus speed multiplier (tiredness, hunger, thirst, exhaustion)
        float multiplier = entityStatus != null ? entityStatus.SpeedMultiplier : 1f;
        moveSpeed = rawSpeed * multiplier;

        if (capsuleHeight > prevHeight)
            transform.position += new Vector3(0f, (capsuleHeight - prevHeight) / 2f, 0f);
    }
    ...
    IEnumerator ClimbOntoObject(Vector3 ledgePos)
    {
        ...
        transform.position = ledgePos + new Vector3(0f, capsuleHeight / 2f + 0.06f, 0f);
        enableMovement = true;
    }
    ...
    {
        if (input != null) { input.PlayerInputMap.Disable(); input.Disable(); }
    }
