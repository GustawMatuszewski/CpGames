...
public class PlayerAnimationsController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        bool currentlyStunned = (combat != null && combat.isStunned);

        // ── PRIORYTET 1: GetHurt — blokuj, nie ruszaj animatora ───────────
        if (currentlyStunned)
        {
            wasStunned   = true;
            wasAttacking = false;
            if (modelTransform != null)
                modelTransform.localRotation = Quaternion.Slerp(
                    modelTransform.localRotation, targetRotation, Time.deltaTime * rotationSpeed);
            return;
        }

        bool attacking     = (combat != null && combat.attackInProgress);
        bool stunJustEnded = wasStunned && !currentlyStunned;

        // Po stunie resetuj lastState — żadne zakolejkowane stany nie odpalają się
        if (stunJustEnded)
        ...
        // ── PRIORYTET 2: Atak — blokuj animacje ruchu ─────────────────────
        if (!attacking)
        {
    ...
    public void PlayHitAnimation(string animName)
    {
        if (string.IsNullOrEmpty(animName)) return;
        animator.Play(animName, 0, 0f);
        lastState = MovementStateController.mState.None;
    ...
    {
        if (animator == null || string.IsNullOrEmpty(animName)) return false;
        AnimatorStateInfo info = animator.GetCurrentAnimatorStateInfo(0);
        return info.IsName(animName) && info.normalizedTime < 0.95f;
    }
    ...
    void PlayAnimationForState(MovementStateController.mState state)
    {
        if (Simple_Animations_Changing_Directions)
        {
            switch (state)
            {
                ...
                or MovementStateController.mState.Crouch_Forward_Left or MovementStateController.mState.Crouch_Right
                or MovementStateController.mState.Crouch_Left:
                ...
                or MovementStateController.mState.Walk_Forward_Left or MovementStateController.mState.Walk_Right
                or MovementStateController.mState.Walk_Left:
                ...
                or MovementStateController.mState.Run_Forward_Left or MovementStateController.mState.Run_Right
                or MovementStateController.mState.Run_Left:
                ...
                or MovementStateController.mState.Sprint_Forward_Left or MovementStateController.mState.Sprint_Right
                or MovementStateController.mState.Sprint_Left:
        ...
        else
        {
            switch (state)
            {
                case MovementStateController.mState.Jumping:      SafeCrossFade(anim_Jump);         break;
                case MovementStateController.mState.Idle:         SafeCrossFade(anim_Idle);         break;
                case MovementStateController.mState.Crouch:       SafeCrossFade(anim_Crouch);       break;
                case MovementStateController.mState.Walk_Forward: SafeCrossFade(anim_Walk_Forward); break;
                case MovementStateController.mState.Run_Forward:  SafeCrossFade(anim_Run_Forward);  break;
            }
        }
    }
